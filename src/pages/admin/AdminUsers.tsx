...
export default function AdminUsers() {
  ...
  return (
    <div className="space-y-6">
                ...
                <tr key={u.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => openEdit(u)}
                        className="text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 text-xs font-semibold px-2.5 py-1.5 rounded-lg transition-colors"
                      >
                        Edit
                      </button>
                      <button
          ...
          {filtered.map((u, idx) => (
            <div key={u.id} className="p-4 space-y-3">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => openEdit(u)} className="flex-1 text-center text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 text-xs font-semibold py-1.5 rounded-lg">Edit</button>
            ...
            {viewLoading ? (
              ...
              <>
                ...
                <div className="px-6 pb-6 flex gap-3">
                  <button onClick={() => { setViewUser(null); openEdit(viewUser); }} className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2.5 rounded-xl text-sm transition-colors">Edit User</button>
      ...
      {editUser && (
        <Backdrop onClose={() => setEditUser(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col overflow-hidden max-h-[90vh]">
            <div className="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-indigo-700 to-indigo-500">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center text-sm font-bold text-white">
                  {getInitials(editUser.userName)}
                </div>
                <div>
                  <h3 className="text-base font-bold text-white">Edit User</h3>
                  <p className="text-indigo-100 text-xs mt-0.5">@{editUser.userName}</p>
                </div>
              </div>
              <button onClick={() => setEditUser(null)} aria-label="Close" className="w-8 h-8 rounded-lg bg-white/20 hover:bg-white/35 flex items-center justify-center transition-colors">
                <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
