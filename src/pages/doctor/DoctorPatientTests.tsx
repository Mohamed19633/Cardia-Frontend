...
export default function DoctorPatientTests() {
  ...
  const handleWritePrescription = () => {
    navigate('../prescription', {
      state: { patientId, patientName, patientInitials, avatarClass },
    });
  };

  const handleViewPrescription = async (pred: PredictionItem) => {
    if (!patientId || pred.id == null) return;
    setPrescriptionModal({ predId: pred.id, data: null, loading: true, error: '' });
    try {
      const res = await getPrescriptionByPrediction(patientId, pred.id);
      setPrescriptionModal({ predId: pred.id, data: res.data, loading: false, error: '' });
    } catch {
      setPrescriptionModal({ predId: pred.id, data: null, loading: false, error: 'Failed to load prescription.' });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {!loading && predictions.map((pred, i) => {
        const riskLabel = pred.riskScore ?? 'Unknown';
        const badgeCls = RISK_BADGE[riskLabel] ?? 'bg-gray-100 text-gray-700 border-gray-200';
        const isPositive = pred.result?.toLowerCase().includes('heart disease') &&
          !pred.result?.toLowerCase().includes('no');
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-wrap gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">{pred.DateAndTime}</p>
                </div>
              </div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {isLatest ? (
                  ...
                  <button
                    onClick={() => handleViewPrescription(pred)}
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-slate-50 border border-gray-100 rounded-xl p-4">
                  ...
                  <p className="text-sm font-semibold text-gray-800">{pred.riskScore}</p>
                </div>
                <div className="bg-slate-50 border border-gray-100 rounded-xl p-4">
                  <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Result</p>
                  <p className="text-sm font-semibold text-gray-800">{pred.result}</p>
                </div>
                <div className="bg-slate-50 border border-gray-100 rounded-xl p-4">
                  <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1.5">Recorded For</p>
                  <p className="text-sm font-semibold text-gray-800">{pred.belongsTo}</p>
                </div>
              </div>
              <div>
